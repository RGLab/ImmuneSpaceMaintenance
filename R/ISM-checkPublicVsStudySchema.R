...
#' @export
checkPublicVsStudySchema = function() {
  # Data from Immport schema (shown in study overview before login)
  pub <- labkey.selectRows(baseUrl = self$config$labkey.url.base,
                           folderPath = "/Studies/",
  ...
  fcsSampleFiles <- labkey.executeSql(baseUrl = self$config$labkey.url.base,
                                      folderPath = "/Studies/",
                                      schemaName = "study",
                                      sql = gsub("tmp", "fcs_sample_files", base))
  fcsSampleFiles$Label <- "FCS sample files"

  fcsControlFiles <- labkey.executeSql(baseUrl = self$config$labkey.url.base,
                                       folderPath = "/Studies/",
                                       schemaName = "study",
                                       sql = gsub("tmp", "fcs_control_files", base))
  fcsControlFiles$Label <- "FCS control files"

  geFiles <- labkey.executeSql(baseUrl = self$config$labkey.url.base,
                               folderPath = "/Studies/",
                               schemaName = "study",
